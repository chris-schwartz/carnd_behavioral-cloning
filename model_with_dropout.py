...
from keras.layers import Dense, Flatten, ELU, Convolution2D, Lambda, Dropout, MaxPooling2D
...
flags.DEFINE_integer('batch_size', 256, "Batch size for each generation")
...
def parse_driving_log(directory):
    ...
    with open(directory + '/driving_log.csv') as csvfile:
        ...
        for row in reader:
            if is_header:
                ...
                # reserve 10% of images for validation
                if random.randint(1, 10) == 1:
                    ...
                    val_image_paths.append(left_img)
                    val_steering_angles.append(steering_angle + STEERING_OFFSET)

                    right_img = directory + '/' + row[RIGHT_IMAGE_COLUMN_IDX].strip()
                    val_image_paths.append(right_img)
                    val_steering_angles.append(steering_angle - STEERING_OFFSET)
    ...
    val_image_paths, val_steering_angles = shuffle(val_image_paths, val_steering_angles)
...
def generate_samples(image_paths, steering_angles, batch_size=64):
    """
    ...
    while True:
        ...
        while batch_idx < batch_size:
            ...
            # apply x,y translation to 50% of images
            if random.randint(0, 10) < 5:
                img, steering_angle = shift(img, steering_angle, shift_scale=0.3)

            # random brightness adjustment between 50% and 125% of original brightness
            img = random_brightness_adjustment(img, min_brightness_scale=0.10, max_brightness_scale=1.10)
...
def main():
    ...
    x, y = next(generate_samples(image_paths[0:1], steering_angles[0:1]))
    ...
    # train model
    run('training model', model.fit_generator)(generate_samples(image_paths, steering_angles, BATCH_SIZE),
                                               samples_per_epoch=SAMPLES_PER_EPOCH,
                                               nb_epoch=EPOCHS,
                                               validation_data=generate_samples(val_image_paths,
                                                                                val_steering_angles, BATCH_SIZE),
                                               nb_val_samples=1000,
